...
# Configuration:
#  Auth0 Account data:
AUTH0_DOMAIN = 'dev-cafeshop.eu.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'cafe'
...
'''
@DONETODO implement get_token_auth_header() method
...
'''
@DONETODO implement verify_decode_jwt(token) method
    @INPUTS
...
def verify_decode_jwt(token):
    ...
    if rsa_key:
        try:
            # Use the key to validate JWT & DECODE:
            payload = jwt.decode(
                token,
                rsa_key,
...
'''
@DONETODO implement @requires_auth(permission) decorator method
    @INPUTS
        permission: string permission (i.e. 'post:drink')
...
def requires_auth(permission=''):
    def requires_auth_decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            # print('REQUIRES AUTH TOKEN: ', token)
            payload = verify_decode_jwt(token)
            # Check if that permission exists in the permission jwt:
